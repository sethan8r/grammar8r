...
AI даёт описание человека или предмета по-русски с пропуском: "Твоя подруга Лена учится на врача. ___ очень умная." Пользователь вводит нужное местоимение. AI проверяет выбор и объясняет, если ошибка — почему именно he/she/it/they.
...
AI даёт русское предложение (например: "Я — учитель." / "Она устала." / "Мы дома."). Под предложением — карточки-подсказки с ключевыми словами (teacher, tired, at home), поскольку пользователь может их не знать(то бишь ии еще присылает слова, которые могут понадобятся, но при это не присылает местоимения и to be). Пользователь пишет полный английский перевод: "I am a teacher." / "She is tired." / "We are at home." AI проверяет форму am/is/are и объясняет ошибку.
...
AI даёт одно утвердительное предложение с to be (например: "She is at home."). Задача: написать его в отрицательной форме. AI проверяет правильность am not / isn't / aren't и объясняет ошибку. Затем кнопка "Ещё задание" — AI даёт следующее предложение.
...
AI даёт одно утвердительное предложение (She is tired. He is at work.). Задача: переделать в вопрос и дать краткий ответ (Yes/No на выбор пользователя). AI проверяет порядок слов в вопросе и форму краткого ответа. Затем кнопка "Ещё задание".

*Примечание:* слова в тексте от AI можно нажать для получения перевода.
...
AI даёт русское предложение и частичный английский перевод с пропуском:  
RU: В комнате стоит диван. → EN: ___ a sofa in the room.  
Пользователь вписывает "There is" или "There are". AI проверяет выбор и объясняет ошибку.  
...
AI даёт русское предложение и его английский утвердительный перевод:  
RU: В кафе есть свободные столики. → EN: There are free tables in the café.  
Задача: написать (1) отрицание и (2) вопрос из этого предложения.  
...
AI даёт набор слов: местоимение + существительное (например: "they / two cats"). Задача: составить предложение "They have two cats." AI проверяет форму have/has и объясняет ошибку.
...
AI даёт утвердительное предложение с have/has (например: "He has a new laptop."). Задача: написать отрицательную форму. AI проверяет форму doesn't have и объясняет ошибку.
...
AI даёт русский вопрос (например: "У твоей подруги есть машина?") и показывает слова-подсказки: girlfriend — подруга, car — машина, job — работа, brother — брат, sister — сестра, ticket — билет. Задача: написать вопрос на английском с правильным Do/Does. AI проверяет форму и объясняет ошибку.
...
AI даёт русское слово и английское существительное (например: "яблоко → apple", "машина → car", "вода → water", "апельсин → orange"). Задача: выбрать a, an или прочерк (—) — нет артикля. AI проверяет и объясняет ошибку. Особое внимание: water, milk, music → прочерк (неисчисляемые).
#### Clarification Options
...
AI даёт предложение с пропуском перед прилагательным или существительным-ловушкой (например: "He is ___ old man." / "She went to ___ European university."). Пользователь вписывает a или an. AI проверяет с объяснением почему именно этот вариант.
...
AI генерирует английское предложение с 2–3 пропусками на месте артиклей и даёт его русский перевод. Приложение показывает пропуски как поля ввода `___` прямо в строке текста. Пользователь вписывает `a` / `an` / `the` / `-` в каждое поле. Заполненное предложение целиком отправляется AI — AI проверяет каждый артикль и объясняет ошибку если она есть.
...
AI даёт предложение с 2–3 пропусками для притяжательных местоимений и русский перевод. Пример задания:
> "___ brother is a doctor. ___ name is Alex." *(Её брат — врач. Его зовут Алекс.)*
...
AI даёт предложение с пропуском + русский перевод. Пример задания:
> "That jacket is ___, not ___." *(Та куртка её, а не моя.)*
...
AI даёт 3 предложения с пропусками + переводы. Пример задания:
> "___ dog is very big." *(Собака Саши очень большая.)*
> "What is ___ this street?" *(Как называется эта улица?)*
...
AI даёт ситуацию + 2–3 предложения с пропусками и переводом. Пример задания:
> "___ is my coffee." *(Вот этот кофе — мой. Стоит передо мной.)*
> "___ restaurant looks great." *(Тот ресторан выглядит отлично. Показываешь вдаль.)*
...
AI даёт 2–3 предложения с пропусками и переводом. Пример задания:
> "___ keys are mine." *(Вот эти ключи — мои. Лежат рядом.)*
> "___ buildings are very old." *(Те здания очень старые. Показываешь вдаль.)*
...
AI даёт 2–3 предложения с пропусками вопросительных слов и переводом. Пример задания:
> "___ is your favourite season?" *(Какое твоё любимое время года?)*
> "___ is the nearest café?" *(Где ближайшее кафе?)*
...
AI даёт 2-3 отдельных предложения с пропусками и переводом. Пример задания:
> "___ eople are in your family?" *(Сколько человек в твоей семье?)*
> "___ is this bag?" *(Сколько стоит эта сумка?)*
