...
> ⚠️ ДЕТАЛЬНАЯ СТРУКТУРА — источник правды в `grammar8r_plan.md` → дерево темы «Наречия»
> (5 микротем МТ72–76 / 17 карточек Card 211–227, с дедупом от MT32 и правилами слива слов).
> Ниже — исходный черновой набросок брифа (общий смысл сохранён, но карточки/номера — по дереву плана).
...
> ⚠️ СПИСОК НИЖЕ УСТАРЕЛ (22.08.2026). Составлен ДЕТАЛЬНЫЙ ПЛАН — источник правды теперь дерево темы
> в `grammar8r_plan.md` (раздел «📁 Present Perfect: Simple и Continuous» → `Present Perfect Continuous`,
> Topic 24, МТ151–157, Card 400–417). Что изменилось против списка ниже:
> • Вводной карточки-приоритета НЕТ (канон с 26.06.2026), и новых неправильных глаголов у темы тоже
>   нет — считаем известным весь диапазон колоды id 1–100.
> • Добавлен восьмой образ ряда метафор курса («дубль, который ещё не остановили») и обязательная
>   карточка-контраст «почему русские ошибаются».
> • Добавлены два употребления, которых в списке не было: процесс только что оборвался, а следы налицо
>   (+ оценочная интонация PPC), и полоса повторов в последнее время.
> • Тема закрывает два долга курса: третья роль `been` (из «Был и вернулся ↔ уехал») и for/since
>   с длительной формой (из «Сколько это длится»).
> • Разница с Present Perfect — предпоследняя микротема на 5 карточек, финал — Live Practice.
...
> ⚠️ ОБЪЕДИНЕНО В РАЗДЕЛ (06.08.2026): §10 (will), §11 (going to) и тема `Compare: will vs going to`
> живут внутри одного сворачиваемого раздела «📁 Future · Будущее время» (GrammarTopicCategory) —
> см. `grammar8r_plan.md`. Содержание брифа актуально, изменилась только упаковка: снаружи будущее
> читается одной строкой, внутри три темы (will → going to → Compare). Закрывающая карточка
> Compare — мост во вкладку «Практика» на смешанную тренировку пройденных времён.
...
> ⚠️ РАЗДЕЛЕНО НАДВОЕ (23.07.2026): **базовая** версия темы («Passive (базовый) · be + V3», 4 карточки)
> вынесена НАМНОГО раньше — после Past Continuous, перед Linking Words (см. `grammar8r_plan.md`).
> Там даётся идея, формула `be + V3` в Present/Past Simple, отрицание и вопрос без did, узнавание
> `by`, разведение «трёх was» и бытовые формулы (I am called…, I was told, Made in Italy).
> Эта, полная тема остаётся на своём месте, но начинается с КОРОТКОГО напоминания, а основной объём
> отдаёт перфектным/модальным пассивам, полному разбору by и get-passive. Список ниже — исходный,
> дедуп смотреть в плане.
...
> перед Future Simple) — см. `grammar8r_plan.md`.
>
> ⚠️⚠️ СПИСОК НИЖЕ УСТАРЕЛ (02.08.2026). Составлен ДЕТАЛЬНЫЙ ПЛАН — источник правды теперь дерево
> темы в `grammar8r_plan.md` («Linking Words · Связная речь», Topic id=17, MT98–107, Card 280–301).
> Что изменилось против списка ниже:
> • Тема называется «Linking Words · Связная речь» и заводит НОВУЮ категорию слов
>   `linking · Связная речь` (её потом расширяют So/Such и Too/Enough, Conditionals,
>   Дополнительные конструкции — пометки проставлены в тех темах).
> • Добавлена вводная микротема-концепт MT98 с ОСЬЮ темы: что идёт после связки — целая часть
>   с подлежащим или существительное/-ing. Добавлена финальная микротема Live Practice (MT107)
>   с явным инвентарём связок в промте.
> • Пункт 5 «Добавление: also/too/as well» СИЛЬНО урезан: also/too/as well уже разобраны ПОЛНОСТЬЮ
>   в Основах Card 62 — повторять запрещено, берём только недосказанное (too = «слишком»,
>   Me neither, Also, в начале предложения) + формальный ярус moreover/furthermore/in addition.
> • Пункт 4 «Despite / In spite of» расширен (02.08.2026): микротема стала «Despite / Instead ·
>   Вопреки и вместо», третьей карточкой добавлен `instead of` / `instead` — первое знакомство,
>   слова в курсе не было ни разу. Причина: та же порода, что у despite (+ noun/-ing), и лучшая
>   контрастная пара темы — «несмотря на» ↔ «вместо». Из слива «Дополнительных конструкций»
>   instead of убран, там остаётся только rather than.
> • Действует КАНОН ОСВЕЖЕНИЯ: половина связок уже роздана по курсу — такие карточки напоминают
>   коротко (что это и где используется, 1–2 строки), а разбор дают только тому, чего не говорили.
> • Пометки для автора: could в примерах цели — превью-строкой (разбор в Модальных глаголах);
>   "It's been a while!" — застывшая фраза с плашкой про Present Perfect.
