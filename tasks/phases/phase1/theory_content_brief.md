...
- `_ex1` — всегда 1, одно AI упражнение на карточку
...
- `aiExercise` — обязательно. Четыре поля в assets JSON:
  - `id` — уникальный строковый ключ упражнения. Формат: `"<microtopic>_card<N>_ex1"`. Пример: `"articles_card1_ex1"`, `"present_simple_card3_ex1"`. **Этот же id — ключ промта на сервере.** Должен быть уникальным глобально.
