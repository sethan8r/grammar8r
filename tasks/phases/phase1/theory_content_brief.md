  ...
  "cards": [
    {
      ...
      "theorySummary": [{ "type": "paragraph", "text": "Краткое правило блоками, как theory — показывается по кнопке «?»." }],
      "order": 1,
      "examples": [
...
- `theorySummary` — краткое правило блоками (суть сверху, дальше блоки по надобности). Подсказка по кнопке «?». Канон — `tasks/theory_content_guide.md` §3 → «`#### Summary` — краткое правило БЛОКАМИ».
...
> ⚠️ СПИСКИ §12–13 НИЖЕ УСТАРЕЛИ (25.09.2026). Future Continuous, Future Perfect и Future Perfect
> Continuous объединены в раздел «📁 Future: Continuous и Perfect» — источник правды теперь дерево
> в `grammar8r_plan.md`. У Future Continuous там собрано полное дерево (10 микротем, инвентарь ролей +
> три независимые проверки), у Future Perfect — тоже (9 микротем, собрано 26.09.2026), у Future
> Perfect Continuous — дерево пересобрать перед написанием. Вводной
> карточки-приоритета НЕТ (канон 26.06.2026).
...
> ⚠️ ДЕРЕВО ТЕМЫ ЖИВЁТ В `grammar8r_plan.md` (10 микротем / ~30 карточек, собрано прогоном
> инвентаря ролей по гайду §3). Здесь его копии НЕТ — две копии разъедутся. Перед написанием
> читать шапку темы в плане: принцип-корень, ось «два этажа», второй корень (прошедшая форма =
> дистанция), пять долгов по could/would, три обещания, список уже разобранного (Правило №0),
> чужие дома и слив.

Коротко о месте темы в курсе: модальными пользователь к этому моменту уже пользуется — курс весь
год выдавал их готовыми формулами (`I can swim`, `I have to work`, `Could you help me?`,
`If I were you, I'd…`, `It might rain`, `I've got to go`). Тема не учит их заново, а объясняет
изнутри то, что человек уже говорит, и добирает второй этаж значения (догадка: `He must be tired`),
который русскому не приходит в голову вовсе.
...
>
> ⚠️⚠️ СПИСОК НИЖЕ УСТАРЕЛ (20.09.2026). Составлено ПОЛНОЕ дерево прогоном ИНВЕНТАРЯ РОЛЕЙ
> (гайд §3) — источник правды теперь `grammar8r_plan.md` → «Passive Voice · Страдательный залог:
> ЧАСТЬ ВТОРАЯ» (10 микротем / ~25 карточек). Что изменилось против семи пунктов ниже:
> • появились вводная микротема-концепт («это вторая часть темы») и финальная Live Practice;
> • принцип-корень: V3 застывает, двигается только `be` — тема не учит десять форм, а ставит
>   знакомые формы `be` перед неизменным V3;
> • добавлены роли, которых в списке не было вовсе: `going to`-пассив · Continuous-пассив и
>   разведение двух `being` · ПУСТЫЕ КЛЕТКИ (`will be being` ✗, `has been being` ✗) с затравкой
>   `will have been done` · модальный + `have been` + V3 · пассивный инфинитив `to be done` и
>   `needs cleaning` · пассив с двумя дополнениями (`I was given a book`) и `I was explained` ✗ ·
>   предложный и фразовый пассив (`was sent for`, `was called off`) · `by` ≠ `with` · семья
>   `be made of / from / with / in` · вопрос к деятелю · поведение get-пассива с `did` · ГРАНИЦЫ
>   (непереходные, эргативы `The door opened`, состояние ↔ действие) · безличный пассив
>   `It is said that…` и выбор залога;
> • слив ЕСТЬ (17 слов в `useful_words`) — прежнее «слива нет и здесь» отменено: запрет касался
>   категории под грамматику, а не слов, стоящих внутри конструкции;
> • тема закрывает четыре долга: две клетки из «Модальных» (Card 637), пассивный герундий и
>   `needs cleaning` из «Герундия», анонсы из «Passive (базовый)» Card 279, `got fired` из
>   «get — часть 1». И сама ставит долги в «Relative Clauses», «Future Perfect», «Future Continuous».
...
> ⚠️⚠️ СПИСОК НИЖЕ УСТАРЕЛ (23.09.2026). Дерево собрано прогоном инвентаря ролей — источник правды
> теперь `grammar8r_plan.md` → «Relative Clauses» (7 микротем / ~15 карточек). Что изменилось:
> • вводная микротема и финальная Live Practice; принцип-корень «роль связки внутри придаточного
>   решает всё» с опорой на Card 346 («Косвенные вопросы»);
> • добавлены: `that ≠ what` («всё, что», «то, что»), предлог в конце (`the flat I live in`),
>   проверка «там» для where, when / why / `the way`, whose как «у которого», сокращение через
>   V3 / `-ing` / `to` (долги Passive Card 684 и Герундия Card 632), `which` про всю фразу;
> • слив ЕСТЬ (~14 слов в `useful_words`, включая `whom`).
...
> ⚠️⚠️ СПИСОК НИЖЕ УСТАРЕЛ (23.09.2026). Дерево собрано прогоном инвентаря ролей — источник правды
> теперь `grammar8r_plan.md` → «Reported Speech · Косвенная речь» (10 микротем / 22 карточки). Что изменилось:
> • вводная микротема и финальная Live Practice; принцип-корень «пересказ смотрит из сегодняшнего дня,
>   поэтому время отъезжает на шаг назад»;
> • `will → would` и тройка будущего здесь не разбираются заново — они уже даны в «Модальных» (Card 653)
>   и Conditionals (Card 498);
> • добавлены: модальные и первый тип условных в пересказе · мысли тоже сдвигаются (`I thought you were
>   busy`) · когда сдвига нет · `told that` ✗ / `said about` ✗ · speak ↔ talk · человек вплотную или
>   через `to` · пересказ ответов · глаголы пересказа и deny ↔ refuse ↔ reject · источник и слухи ·
>   устный рассказ (`goes` / `be like`);
> • слив ЕСТЬ (20 слов в `useful_words`).
...
> перед Past Perfect) — см. `grammar8r_plan.md`.
>
> ⚠️⚠️ СПИСОК НИЖЕ УСТАРЕЛ (25.08.2026). Составлен ДЕТАЛЬНЫЙ ПЛАН — источник правды теперь дерево
> темы в `grammar8r_plan.md` («Make vs Do · Два английских «делать»», Topic id=25, МТ158–162,
> Card 418–428). Что изменилось против списка ниже:
> • Тема закрывает make/do НАСМЕРТЬ — возвращаться к ним в курсе больше не будем. Поэтому сюда же
>   переехали фразовые make up / make out / do up / do without / do over (вычеркнуты из микротемы
>   «Фразовые глаголы» темы «Устойчивые выражения и идиомы») и make up your mind.
> • Вместо «правила нет, учи устойчивые сочетания» дана ОСЬ: make = появилось нечто, чего не было;
>   do = возишься с тем, что уже есть (make dinner ↔ do the dishes). Ось закрывает ~80% сочетаний,
>   остальное честно называем застывшими формулами.
> • Добавлен железный ключ: неопределённое слово (something / nothing / what) → всегда do.
> • Добавлены роли, где «делать» не переводится вообще: make + состояние (make me happy),
>   make = составлять/получаться (They make a good couple), make it (успеть, справиться),
>   do = годиться (That'll do), застывшие make sense / make sure / do the trick / Well done!.
> • Добавлена карточка-озарение о номинализации: почему носитель говорит make a decision вместо
>   decide, и что «пустых» глаголов четыре — make, do, have, take (прямая параллель с русским
>   «принять решение»). Отсюда же — карточка про take a photo / give a speech и развилка BrE/AmE.
> • Слив слов есть, в существующую категорию `collocations · Слова из связок`, и только партнёрами
>   make/do (decision, effort, mess, housework, difference, sense, trick, couple, choice, story,
>   favour, exam). Финал темы — Live Practice с 3 AI.
...
>
> ⚠️⚠️ СПИСОК НИЖЕ УСТАРЕЛ ЦЕЛИКОМ (08.09.2026). Составлено ПОЛНОЕ дерево микротем — источник
> правды теперь `grammar8r_plan.md` → «Глагол get — часть 1: базовые значения». Что изменилось
> против четырёх пунктов ниже:
> • Появилась ОСЬ темы (вводная микротема-концепт): у get нет своего значения — смысл задаёт то,
>   что стоит СПРАВА. Одиннадцать слотов таблицей, включая роли части 2 (анонсом).
> • У «get = становиться» есть ЯДРО: `get + прилагательное` = русское «-ться / -еть»
>   (hungry → get hungry, lost → get lost), а из него — ловушка be ↔ get как «состояние ↔ момент
>   перехода» (I am lost ↔ I got lost). Разбор идёт парами прилагательное → русский глагол.
> • Добавлены слоты оси, которых в списке не было вовсе: `get + объект + прилагательное`
>   (get it right, get everything ready — ступень к causative) и `get + объект + место`
>   (Can you get me to the airport?).
> • Добавлены: формы got/gotten и ловушка got ↔ have got · get в Continuous (It's getting late)
>   как контраст к state verbs · «стать» ≠ только get (become / go / turn / grow) · предлоги при
>   get + причастие (get married TO) · get = зарабатывать (get paid) · get somebody something ·
>   двузначность `get to work` (добираюсь ↔ приступим) · финальная Live Practice.
> • Пункт 4 списка ниже («get + объект: get up, get dressed») распущен: get up — чужой дом
>   («Verb Particles»), get dressed / get ready ушли в микротему «становиться».
...
18. **Глагол get — часть 2: продвинутые значения** ← продолжение раздела 18.4 (базовые значения)
    ⚠️⚠️ ОПИСАНИЕ НИЖЕ УСТАРЕЛО (08.09.2026). Составлено ПОЛНОЕ дерево микротем — источник правды
    теперь `grammar8r_plan.md` → «Глагол get — часть 2: продвинутые значения + Causative have/get».
    Что изменилось: тема перестала быть «causative плюс две строки» и разложена на микротемы —
    «Get = понимать» (I don't get it, You don't get it как упрёк) и «раздражать» · Causative
    (ловушка №0 «-ся» → форма have/get + obj + V3 → полный набор форм → четвёрка make/get/have/let →
    развод с get-passive) · `get to do` = «довелось» (I got to meet him) · застывшие фразы, включая
    добавленные `Don't get me wrong`, `You got me`, `I get the feeling`, `getting nowhere`,
    `get hold of`, `What are you getting at?` · регистр (где get неуместен: receive / obtain /
    become / arrive) · Live Practice с 3 AI. Вместе с частью 1 тема закрывает ВСЕ роли get —
    что не разбирается, то стоит с указателем на чужой дом (Verb Particles, Passive, Small Talk,
    Разговорный, Доп. конструкции).
...
упражнения на распознавание и применение. Слова синкаются в Words8r: идиомы и устойчивые обороты →
категория "Идиомы и устойчивые обороты"; фразовые глаголы → "Слова из двух" (общая колода с темой
«Verb Particles»).
...
#### Микротема: Тело и самочувствие (добавлена 11.09.2026)

По порядку в теме стоит сразу после «Вежливости и разрешения». Как сказать, что болит, что с телом «само что-то делает» и где русская фраза ведёт в кальку: `I have a sore throat`, `my leg fell asleep`, `my arm went numb`, `I lost my voice`, `I feel sick`, `I have a temperature`. Ось — английский по-разному решает, КТО действует: «имеет» боль (have), даёт части тела болеть самой (hurt) или делает её живой (fell asleep, growling). Карточки, указатели без дубля и слив — `grammar8r_plan.md` → «Фразы из жизни» (источник правды).
...
- **Summary** — краткое правило блоками для кнопки «?» во время упражнений (канон — `tasks/theory_content_guide.md` §3 → «`#### Summary` — краткое правило БЛОКАМИ»)
