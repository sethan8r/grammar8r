...
Определяет откуда брать слова пользователя для формирования промта. Логика проверки минимума и формирования списка слов живёт в `WordsRepository` — он смотрит на `categoryName`, `minWords`, `targetWords` и делает нужный запрос к Words8r через ContentProvider.
...
data class WordArrangementExercise(
    ...
    // TODO: перенумеровать все упражнения в теории по типам (сейчас сквозная нумерация — временно).
