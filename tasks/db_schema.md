...
1. **`md_to_json.py`** — один парсер на ВСЕ theory-файлы (вход = MD-файл аргументом).
   ...
   ⚙️ **`order` микротем и карточек парсер считает сам** — по позиции блока в MD: первая микротема
   файла → 1, первая карточка микротемы → 1, счёт карточек сбрасывается на каждой микротеме.
   В MD эти поля не пишутся вообще. Руками задаётся только `order` ТЕМЫ (её место в курсе из файла
   не выводится) и `order` раздела в строке `**Раздел:** … order=K`. Как вставлять микротему или
   карточку в середину — `theory_content_guide.md` → §2 «ПОРЯДОК СЧИТАЕТ КОНВЕРТЕР».
   ⚠️ `order` — **не идентичность**: он лежит только в content.db, которая пересобирается целиком,
   и `user.db` на него не ссылается ничем. Поэтому перенумеровать `order` безопасно в любой момент,
   а вот `id` обязан быть стабильным навсегда — на него завязан прогресс пользователя.
...
| `dialog` | Диалог-чат (реплики пузырями) | `lines: [{speaker, text, note?}]` |
...
`dialog.lines` — реплики по порядку. `speaker` — имя говорящего из MD (`@Kate:`); служебное имя
`Me` означает нашу сторону и рисуется у правого края залитым акцентом, остальные — у левого на
`Elevated`, с подписью имени над первой репликой говорящего подряд. `note` (необязательный) —
короткий ярлык хода разговора («согласился и добавил своё»), рендерится внутри пузыря рамкой без
заливки. Синтаксис в MD (`@Имя: реплика // ярлык`) и когда ярлыки уместны — `theory_content_guide.md` §8.
...
data class AiExercise(
    ...
    // ⚠️ Связь one-to-many: на одной карточке может быть 1–3 AI-упражнения (id …_ex1/_ex2/_ex3)
    //    или ни одного. cardId НЕ уникален — клиент (дерево Практики, экран карточки) обязан
    //    уметь показать несколько AI на карточку. (Канон 25.06.2026, theory_content_guide.md §5.)
...
data class UserExerciseResult(
    ...
    // TODO(rename): имя от старой логики, по смыслу — solvedOnFirstPass. Переименовать попутно
    // с ближайшей миграцией user.db, отдельную ради имени не заводим.
)
// PK составной: (cardId, exerciseType, exerciseId).
```

> **Анти-чит:** строку создаёт ПЕРВЫЙ ответ (`INSERT OR IGNORE`) — в том числе неверный, чтобы выход
> из карточки его не отменил. Верный ответ со ВТОРОЙ попытки того же захода поднимает строку до
> верной (`UPDATE … SET correctFirstTry = 1`, только на повышение); право на это есть лишь у сессии,
> которая строку создала (список живёт в памяти ViewModel). Перезаход в карточку статистику не меняет
> вообще — упражнения перепроходятся как тренировка.
> Плашки (нереализованный тип / умное задание) сюда НЕ пишутся. Точность по хардкоду больше нигде
> не хранится, а у AI-заданий её нет вовсе — оценка пользователю там отменена (см. `DailyStats`).
...
data class DailyStats(
    ...
    // ⚠️ Поля avgScore здесь НЕТ (решение 20.09.2026): оценка пользователю в AI-заданиях отменена
    //    целиком — ни балла, ни "Точность: X%". Человек видит вердикт «верно / неверно» с фидбеком
    //    в момент ответа, а на экране статистики — только счётчик выполненного.
    //    Обоснование — grammar8r_plan.md → ⚠️-блок «НИКАКОЙ ОЦЕНКИ ПОЛЬЗОВАТЕЛЮ В AI-ЗАДАНИЯХ».
    //    Статистика верных/неверных по ai_exercise.id ведётся на СЕРВЕРЕ и в приложение не попадает.
)
```
...
> **ПРИНЦИП: дерево Словаря зеркалит дерево теории.** Раздел/тема теории → раздел/категория
> Словаря в том же порядке. По умолчанию так и есть (конвертер генерирует группы/категории из
> заголовков theory-файлов). Отклоняемся только осознанно, точечно:
> - **Времена** — НЕ дробим на категорию-на-время (темы времён мелкие, своей верхнеуровневой ветки
>   у них нет). Все времена сливают слова в ОДНУ категорию «Слова из времён». Исключение внутри
>   исключения — «Наречия»: своя категория.
> - **Неправильные глаголы** — наоборот, поднимаем в СОБСТВЕННЫЙ верхнеуровневый раздел (важная
>   тема, видна сразу), а не прячем внутрь «Устройство языка». См. `irregular_verbs` ниже.
> - **`two_word` · «Слова из двух»** — верхнеуровневая строка Словаря, собирающая слова НЕСКОЛЬКИХ
>   тем по общему принципу: всё, что в русском одно слово, а в английском два-три. Сюда сливают
>   «Verb Particles · Хвостики глагола» (раздел `polysemy`) и фразовые глаголы из «Устойчивых
>   выражений и идиом». Имя расходится с именами тем сознательно: в Словаре человек видит одну
>   строку в списке, и «Хвостики глагола» ему ничего не говорит, а «Слова из двух» доносит идею
>   колоды с одного взгляда.
> - **`idioms` · «Идиомы и устойчивые обороты»** — тема «Устойчивые выражения и идиомы», но без
>   фразовых глаголов (они в `two_word`). Здесь готовые фразы-вставки: `break the ice`,
>   `by the way`, `to be honest`, `it's been a while`, формулы make/do.
> - **`useful_words` · «Полезные слова»** — общая словарная колода нескольких плоских тем, у которых
>   нет своего грамматического дома: «Глагол get» (части 1 и 2), «Смотреть, слушать и понимать»,
>   «Погода», «Глагол have — нестандартные употребления», «Глагол take» (партнёры справа —
>   по тому же принципу, что у get). Отдельные категории на каждую такую тему не заводим —
>   это строки по 15–20 слов, которые только дробят Словарь.
> - **`everyday` · «Everyday English»** — бытовые фразы и лексика ситуаций. Обслуживает ДВЕ темы:
>   «Фразы из жизни» и раздел «Английский в ситуациях» (ресторан, отель, транспорт, дорога…).
>   Раздел ситуаций своей категории не заводит: жанр тот же, а колода на восемь тем-ситуаций
>   развалилась бы на строки по десять слов.
>
> Любое такое отклонение — фиксировать здесь, чтобы дерево не разъехалось с теорией незаметно.
...
data class IrregularVerb(
    ...
    val transcription: String?,// Транскрипция всех 3 форм ОДНОЙ строкой в формате
                               // "[v1]/[v2]/[v3]". Пример: "[gəʊ]/[went]/[gɒn]". Nullable.
                               // Одно поле, НЕ три колонки: у форм произношение часто расходится
                               // (read/read/read, lead/led/led) — три звучания внутри строки.

    val microtopicId: Int,
    // FK → GrammarMicrotopic.id (Int). Ключ выборки для BottomSheet открытия.
    // ⚠️ ОСНОВНАЯ КОЛОДА (id 1–118) = 44 (МТ «Неправильные глаголы»). Открывается
    //    вся разом при прохождении этой микротемы.
    // ⚠️ ХВОСТ (id 119–198, редкое/книжное/производное) = 185 (МТ «Second Circle ·
    //    Второй круг времён пройден»). Хвост открывается ПОЗЖЕ, отдельным событием: в Card 483 юзеру
    //    текстом обещано, что основная колода закрыта, а остальные глаголы открываются
    //    как необязательный добор «по желанию». До Card 483 глаголы id 119+ в колоде
    //    не показываются вообще; после — открыты, но НЕ идут в приоритет показа и НЕ
    //    входят в обязательный прогресс и статистику курса.
    //    Диапазоны — `tasks/words/V1V2V3/irregular_verbs_draft.md` → «Маппинг: время → id».

    val categoryId: String
    // FK → course_categories.id (source = "irregular_verbs").
    // ⚠️ Для ВСЕХ глаголов = "verb_forms" — единая категория словаря, идёт отдельной
    //    темой «Неправильные глаголы (V1/V2/V3)». Завести строку в course_categories
    //    (source="irregular_verbs").
    // ⚠️ Категория сидит в СОБСТВЕННОМ верхнеуровневом разделе словаря (отдельный
    //    course_word_groups, напр. id="verb_forms"), НЕ внутри «Устройство языка» —
    //    тема важная, должна быть видна сразу при открытии слов курса (вверху списка).
)
...
> **Механика учёбы/повторения глаголов (согласовано 25.06.2026, реализация позже).**
> Подробности и UI — `words8r_plan.md` → «Режим изучения форм глаголов». Кратко, чтобы не забыть:
> 1. **Транскрипция — одно поле** `transcription`, формат `[v1]/[v2]/[v3]` (напр. `[gəʊ]/[went]/[gɒn]`).
>    На 3 колонки схему НЕ дробим.
> 2. **Повторение — два способа ввода:** клавиатура (3 поля под V1/V2/V3) и «2×2» (выбор верной формы
>    из вариантов). Та же логика, что в Words8r (наследуется, отдельно не изобретаем).
> 3. **Единый пул** всех изучаемых слов с градацией по `qRep` (через `UserWordProgress`) — старое не
>    забывается. Никакого интервального SRS и никакого хард-гейта форм по счётчику.
> 4. **Учёба по 2 глагола** за заход (не жёстко — хочет ещё, жмёт «учить» снова). На карточке учёбы
>    сразу все 3 формы, самооценка свайпом.
...
data class GrammarTopic(
    ...
    // Теги для поиска через запятую, строчными. Пользователю НЕ показываются.
    // Тег темы — широкий зонт: то, как пользователь назовёт всю тему целиком.
    // Пример: "прошедшее время, что было, вторая форма глагола, рассказать о вчера"
    // Источник — строка **Теги:** в шапке темы в MD. Нет тегов → null.
    // Правила написания — theory_content_guide.md; роль в ранжировании — search_feature_brief.md §8.3.
)
```
...
data class GrammarMicrotopic(
    ...
    val searchKeywords: String?
    // Теги для поиска через запятую, строчными. Пользователю НЕ показываются.
    // Тег микротемы — то, чем она ОТЛИЧАЕТСЯ от соседок по теме (общие слова темы живут
    // в GrammarTopic.searchKeywords и здесь не повторяются).
    // Пример: "прошлая привычка, раньше делал а теперь нет, бывало делал"
    // Источник — строка **Теги:** в шапке микротемы в MD. Нет тегов → null.
)
```
...
data class GrammarCard(
    ...
    // JSON: массив блоков того же формата, что theory (раздел «Блоки теории» выше).
    // Краткое правило карточки — окно по кнопке «?» в упражнениях. Канон блоков — theory_content_guide.md §3 → «#### Summary».
...
| `AnimationCatalogCache` 🔜 | Локальное зеркало `animations/catalog.json` (слот → URL + окно показа) для векторных анимаций. Источник правды — объектное хранилище. Снятая с раздачи анимация должна молча исчезать с фолбэком, неизвестный слот — игнорироваться. Схема не проектировалась: фича ближе к релизу. | features.md → «Векторные анимации (Lottie)»; phases/phase4/phase4_server.md → «Векторные анимации» |
| `NewsPostCache` 🔜 | Локальный кэш ленты «Новости» (чтобы читалась офлайн). Источник правды — сервер, таблица `news_posts`; здесь только зеркало ответа `GET /news`. Прочитанность постов — **не сюда, а в DataStore** (это не прогресс обучения). Схема не проектировалась: фича делается ближе к релизу и до тех пор поплывёт. | features.md → «Новости приложения»; phases/phase4/phase4_server.md → «Новости» |