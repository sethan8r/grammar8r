...
> ⚠️ **ПОРЯДОК РАБОТ — НЕ начинать раньше времени.**
> `md_to_json.py` уже готов (генерит JSON-сиды). А вот **`json_to_db.py` (JSON → content.db) и
> Gradle-интеграцию делаем ТОЛЬКО когда:**
> 1. написана ВСЯ теория курса (не только «Основы») — иначе схема/имена ещё могут меняться;
> 2. заведён Android-проект (есть Room-Entity, имена таблиц, gradle).
> До этого момента финальные имена таблиц/Entity не зафиксированы, и собирать БД рано.
> Сейчас этап — **написание теории**; сборка content.db — отдельный поздний этап.
