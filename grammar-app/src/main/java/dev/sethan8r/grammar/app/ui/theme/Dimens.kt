...
object Dimens {
    ...
    /** Накопленный сдвиг пальцем ВВЕРХ, после которого бар появляется (маленький — появляется легко). */
    val bottomBarScrollShowDistance = 10.dp

    /** Накопленный сдвиг пальцем ВНИЗ, после которого бар прячется (больше показа — прятать труднее). */
