...
class SearchNormalizer @Inject constructor() {
    ...
    /**
     * Предлог или союз — такое слово в заголовки и теги не попадает, поэтому требовать его
     * совпадения нельзя. Короткие латинские токены (`s`, `ed`, `to`) служебными НЕ считаются:
     * в грамматическом курсе это сам предмет поиска.
     */
    ...
    private companion object {
         ...
         */
        val STOP_STEMS = setOf("как", "что", "эт", "так", "сказа")

        /** Русские предлоги, союзы и частицы — в заголовках и тегах их не бывает. */
        val FUNCTION_STEMS = setOf(
            "в", "во", "о", "об", "обо", "на", "к", "ко", "с", "со", "у", "за", "из", "по",
            "до", "от", "для", "при", "над", "под", "про", "без", "через", "и", "а", "но",
            "или", "же", "ли", "бы", "чтоб", "то", "ж", "б",
        )
